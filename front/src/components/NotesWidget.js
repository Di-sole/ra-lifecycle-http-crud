...
class NotesWidget extends React.Component {
	...
	async updateState() {
		const data = await fetch(process.env.REACT_APP_NOTES_URL).then(response => response.json());
	...
	render() {
		...
		<div className="widget">
			<div className="widget-header">
				...
				<button className="btn refresh-btn" onClick={this.updateState}>&#8634;</button>
			</div>
			<div className="widget-body">
				{
					this.state.notes.map((note) => <Note key={note.id} note={note} onDelete={this.handleDelete} />)
				}
			</div>
